...
internal sealed class PlotVariancePipe : IPlottingPipe
{
...
file sealed class VariancePlot : Plot
{
    public VariancePlot(SourceFile sourceFile, IReadOnlyCollection<ChunkingReport> reports)
    {
        AddBars(reports);
        AddLegend(reports);
    ...
    private static Bar CreateBar(ChunkingReport report, int index)
    {
        ...
        return new Bar
        {
            ...
            Value = report.Deduplication.VarianceRatio,
            FillColor = color,
            LineColor = color,
        };
    }
